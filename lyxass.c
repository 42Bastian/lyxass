...
{
  printf("%s\nUsage:",info);
  printf("lyxass [-h|{\"-\"(o fn|r|v|w|s|d|x value|t[l|g|d]|D label[=value])} infile]\n");
}
void help()
{
  printf("%s\n%s",info,
         ...
         "-d                                - data output (no BLL or BJL header)\n"
         "-w                                - enable warnings\n"
         ...
         "                                    w, l and p only of 2,4 or 8 bytes\n"
         "dc.a                              - like dc.b only bytes are translated\n"
         "                                    before storing\n"
         "ds.b/ds.w/ds.l/ds.p               - reserve space in chunks of 1,2,4 or 8 bytes\n"
         "dcb.b/dcb.w/dcb.l/dcb.p val,cnt   - define data , <cnt> times repeated,\n"
         "                                    data and count must be constant\n"
...
//->         "                             empty clears it\n"
         ...
         "rept expression                   - repeat following source (no nesting)\n"
         "endr                              -\n"
         ...
         "fail \"%Dlabel %Hlabel\"            - like `echo` but generates an error\n"
         ...
         "                                    regs not used in either bank are ommited\n"
         ...
         "  precedence.\n"
         "Conditions: Equal is '==' or '=', unequal is '<>'\n"
         "A @ in an expression is replaced by the current @-counter value.\n"
